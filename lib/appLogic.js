...
function compactItemArrays(it){
  ...
  if(it.belt && typeof it.belt === 'object'){
    ['headSlots','leftSlots','rightSlots','bars','arrows'].forEach(k => {
...
export async function initApp(){
  ...
  function syncStationsToSteps(id){
    ...
    // เก็บ "ตำแหน่ง" ที่ผู้ใช้กรอกในตารางไว้ (ค่านี้ไม่ได้มาจากสถานี) เพื่อคืนกลับตามชื่อกิจกรรม
    const prevPos = new Map();
    (ed.steps||[]).forEach(s=>{ const n=((s&&s.process)||'').trim(); if(n && !prevPos.has(n)) prevPos.set(n, s.position||''); });
    const raw = [];
    ...
    // ยุบชื่อกิจกรรมที่ซ้ำกันให้เหลือบรรทัดเดียว (บวกคน + Rate)
    ed.steps = mergeSteps(raw);
  }
  ...
  function renderDetail(id){
    ...
    // ยุบชื่อกิจกรรมซ้ำเสมอตอนแสดงผล (เผื่อ it.steps ยังไม่ถูก sync ให้ยุบ เช่นเมนูที่ยังไม่เคยแก้)
    const steps = mergeSteps(it.steps || []);
    ...
    main.querySelectorAll('#stepsBody input').forEach(inp=>{
      inp.addEventListener('input', (e)=>{
        ...
        const row = mergeSteps(ed.steps || [])[idx];
        if(!row) return;
    ...
    main.querySelectorAll('[data-del]').forEach(btn=>{
      btn.onclick = (e)=>{
        ...
        if(name){
          ...
          ed.steps = mergeSteps(ed.steps || []); ed.steps.splice(idx,1);
        }
  ...
  function setupStepsDnD(id){
    ...
    body.addEventListener('drop', (e)=>{
      ...
      const ed = ensureEdited(id);
      ed.steps = mergeSteps(ed.steps || []); // ให้ index ตรงกับแถวที่แสดง (ยุบรวมแล้ว)
