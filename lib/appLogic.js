...
export async function initApp(){
  // ข้อมูลตั้งต้น ~6 MB และข้อมูลที่แก้ไว้ในฐานข้อมูลอีก ~9 MB
  // ยิงทุกคำขอพร้อมกันตั้งแต่แรก แล้วค่อยรอทีเดียว (เดิมรอทีละอย่างจึงนานมาก)
  setBootMsg('กำลังโหลดข้อมูลเมนู...');
  const boot = Promise.all([
    fetch('/data/payload.json').then(r => r.json()),
    storage.get('categories_list', true).catch(()=> null),
    storage.get('deleted_items', true).catch(()=> null),
    storage.get('custom_items_index', true).catch(()=> null),
    storage.getAll('item:', true).catch(()=> null),
  ]);
  let RAW, catRes, delRes, idxRes, itemRows;
  try{
    [RAW, catRes, delRes, idxRes, itemRows] = await boot;
  }catch(err){
    setBootMsg('โหลดข้อมูลไม่สำเร็จ ลองรีเฟรชหน้าอีกครั้ง');
    const ov = document.getElementById('bootOverlay');
    if(ov) ov.classList.add('failed');
    throw err;
  }
  setBootMsg('กำลังจัดข้อมูล...');
  ...
  /* ---- บันทึกสายพานทั้งไลน์เป็นไฟล์รูป (จะได้ไม่ต้องแคปหน้าจอเอง) ----
     ถ่ายที่ขนาดจริงเสมอ ไม่อิงระดับซูมที่ดูอยู่ และซ่อนปุ่มควบคุมต่าง ๆ ไว้ก่อน
     โหลด html-to-image ตอนกดปุ่ม ไม่รวมไปกับ bundle แรก เพราะหน้านี้เปิดช้าอยู่แล้ว */
  // html2canvas วาดข้อความในช่องกรอกเพี้ยน (ลอยขึ้นบน/โดนตัด) จึงแทนช่องกรอกในสำเนาที่ใช้ถ่ายด้วย
  // กล่องข้อความธรรมดาที่หน้าตาเหมือนกัน — ของจริงบนหน้าจอไม่ถูกแตะต้อง
  function replaceFieldsWithText(doc){
    const origs = document.querySelectorAll('#beltViewport input, #beltViewport textarea');
    const clones = doc.querySelectorAll('#beltViewport input, #beltViewport textarea');
    clones.forEach((el, i)=>{
      const src = origs[i];
      if(!src) return;
      const cs = getComputedStyle(src);
      const multiline = src.tagName === 'TEXTAREA';
      const box = doc.createElement('div');
      box.textContent = src.value || '';
      box.style.cssText = [
        ...
        'justify-content:' + (cs.textAlign === 'center' ? 'center' : 'flex-start'),
        'padding:' + (multiline ? cs.padding : '0 ' + cs.paddingRight + ' 0 ' + cs.paddingLeft),
        'font:' + cs.font,
        'color:' + cs.color,
  ...
  async function exportBeltImage(id){
    const btn = document.getElementById('exportImgBtn');
    const vp = document.getElementById('beltViewport');
    if(!vp) return;
    const it = getItem(id);
    const name = ((it && (it.clean_title || it.title)) || 'สายพาน').trim().replace(/[\\/:*?"<>|]/g, '-');
    const savedTransform = vp.style.transform;
    ...
    try{
      ...
      const canvas = await html2canvas(vp, {
        ...
        onclone: (doc)=> replaceFieldsWithText(doc),
      });
      ...
      showToast('บันทึกรูปแล้ว: ' + name + '.png');
    }catch(err){
      ...
      applyBeltZoom();
      if(btn){ btn.disabled = false; btn.textContent = '🖼 บันทึกเป็นรูป'; }
    }
  }
  ...
  // คอลัมน์ซ้าย/ขวาอยู่กึ่งกลางเลนของตัวเอง (25% / 75%) ไม่ชิดขอบราง — กว้าง 24% จึงวาง 3 เครื่องได้ไม่ทับกัน
  const MACHINE_W = 24;
  const MACHINE_COLS = [25, 50, 75].map(c => c - MACHINE_W / 2);
  ...
  // รับข้อมูลที่โหลดมาแล้ว (ทุกคำขอยิงขนานกันตั้งแต่ต้น initApp) มาใส่ตัวแปรในแอป
  // เมนูที่เพิ่มเองกับเมนูที่แก้ไขแล้ว อยู่ในคีย์ item:<id> ชุดเดียวกัน จึงอ่านจากก้อนเดียวได้ทั้งคู่
  function applyLoadedEdits({catRes, delRes, idxRes, itemRows}){
    try{ if(catRes && catRes.value) CATEGORIES = JSON.parse(catRes.value); }catch(e){}
    try{ if(delRes && delRes.value) DELETED_IDS = new Set(JSON.parse(delRes.value)); }catch(e){}
    const byId = {};
    (itemRows || []).forEach(row=>{
      try{
        const val = compactItemArrays(JSON.parse(row.value));
        if(val && val.id){ byId[val.id] = val; edited[val.id] = val; }
      }catch(e){}
    });
    try{
      if(idxRes && idxRes.value){
        JSON.parse(idxRes.value).forEach(id=>{ if(byId[id]) CUSTOM_ITEMS.push(byId[id]); });
      }
    }catch(e){}
  }
  /* ---- ย้อนกลับ (undo) ----
     ทุกการแก้ไขลงเอยที่ persist() อยู่แล้ว จึงเก็บภาพรวมเมนู "ก่อนบันทึกครั้งนี้" ไว้ตรงนั้นที่เดียว
     ไม่ต้องไปแทรกในทุกปุ่ม/ทุกการลาก เก็บในหน่วยความจำของแท็บนี้เท่านั้น (รีโหลดแล้วหาย) */
  const UNDO_MAX = 30;
  const undoStack = {};   // id -> [JSON ของเมนูก่อนการแก้แต่ละครั้ง]
  const lastSaved = {};   // id -> JSON ล่าสุดที่บันทึก/เปิดดู ใช้เทียบว่ามีอะไรเปลี่ยน
  function snapshotOf(id){
    const it = edited[id] || getItem(id);
    return it ? JSON.stringify(it) : null;
  }
  function rememberState(id){
    if(lastSaved[id] === undefined) lastSaved[id] = snapshotOf(id);
  }
  function canUndo(id){ return !!(undoStack[id] && undoStack[id].length); }
  function updateUndoBtn(id){
    const btn = document.getElementById('undoBtn');
    if(!btn || renderingId !== id) return;
    const n = (undoStack[id] || []).length;
    btn.disabled = !n;
    btn.textContent = n ? '↶ ย้อนกลับ (' + n + ')' : '↶ ย้อนกลับ';
  }
  function undoLast(id){
    const cur = snapshotOf(id);
    let target;
    // ยังพิมพ์ค้างในช่องอยู่ (ยังไม่ blur จึงยังไม่ถูกบันทึก) → ย้อนแค่สิ่งที่พิมพ์ค้างก่อน
    if(lastSaved[id] !== undefined && cur !== lastSaved[id]) target = lastSaved[id];
    else if(canUndo(id)) target = undoStack[id].pop();
    else { showToast('ไม่มีการแก้ไขให้ย้อนกลับแล้ว'); return; }
    edited[id] = compactItemArrays(JSON.parse(target));
    persist(id, 'ย้อนกลับแล้ว', true);
    renderDetail(id); renderSidebar(document.getElementById('searchBox').value); renderCatChips();
  }
  async function persist(id, okMsg, skipUndo){
    const snap = snapshotOf(id);
    if(!skipUndo && lastSaved[id] !== undefined && lastSaved[id] !== snap){
      (undoStack[id] = undoStack[id] || []).push(lastSaved[id]);
      if(undoStack[id].length > UNDO_MAX) undoStack[id].shift();
    }
    if(snap !== null) lastSaved[id] = snap;
    updateUndoBtn(id);
    const data = edited[id] || getItem(id);
    // okMsg === false = บันทึกเงียบ ๆ ใช้ตอนบันทึกหลายเมนูรวดเดียว จะได้ไม่เด้ง toast รัว ๆ
    try{ await storage.set('item:' + id, JSON.stringify(data), true); if(okMsg !== false) showToast(okMsg || 'บันทึกแล้ว'); }
  ...
  function verifiedDateText(it){
    if(!it || !it.verifiedAt) return '';
    const d = new Date(it.verifiedAt);
    return isNaN(d) ? '' : d.toLocaleDateString('th-TH', {day:'numeric', month:'short', year:'2-digit', timeZone:'Asia/Bangkok'});
  }
  function toggleVerified(id){
    const ed = ensureEdited(id);
    // ยกเลิกต้องถามก่อน กันเผลอกดทับสถานะที่ตรวจไว้แล้ว
    if(ed.verified && !window.confirm('ยกเลิกสถานะ "ตรวจสอบแล้ว" ของเมนูนี้?')) return;
    ed.verified = !ed.verified;
    ed.verifiedAt = ed.verified ? new Date().toISOString() : null;
    persist(id, ed.verified ? 'บันทึกว่าตรวจสอบแล้ว ✓' : 'ยกเลิกสถานะตรวจสอบแล้ว');
  ...
  async function deleteMenu(id){
    ...
    delete edited[id];
    delete undoStack[id]; delete lastSaved[id];
  ...
  async function duplicateMenu(id){
    ...
    copy.verified = false; copy.verifiedAt = null; // สำเนายังไม่ผ่านการตรวจ
  ...
  function renderCatChips(){
    ...
    CATEGORIES.forEach(c => {
      html += `<div class="cat-chip ${currentCategoryFilter===c?'active':''}" data-cat="${escapeHtml(c)}">${escapeHtml(c)}` +
        `<button class="cat-chip-act" data-catedit="${escapeHtml(c)}" title="เปลี่ยนชื่อหมวดหมู่นี้">✎</button>` +
        `<button class="cat-chip-act" data-catdel="${escapeHtml(c)}" title="ลบหมวดหมู่นี้">✕</button></div>`;
    });
    ...
    wrap.querySelectorAll('.cat-chip').forEach(chip=>{
      chip.onclick = (e) => {
        if(e.target.closest('[data-catedit]') || e.target.closest('[data-catdel]')) return;
        const c = chip.dataset.cat; currentCategoryFilter = (c==='__all__') ? null : c; renderAll();
      };
    });
    wrap.querySelectorAll('[data-catedit]').forEach(b=> b.onclick = (e)=>{ e.stopPropagation(); renameCategory(b.dataset.catedit); });
    wrap.querySelectorAll('[data-catdel]').forEach(b=> b.onclick = (e)=>{ e.stopPropagation(); deleteCategory(b.dataset.catdel); });
  }
  function itemsInCategory(cat){
    return allItems().map(b => getItem(b.id)).filter(it => (it.category || '') === cat);
  }
  // เปลี่ยนชื่อ/ลบหมวดหมู่ ต้องไล่แก้เมนูที่อยู่ในหมวดนั้นด้วย ไม่งั้นเมนูจะอ้างหมวดที่ไม่มีอยู่แล้ว
  async function applyCategoryChange(cat, next, okMsg){
    const affected = itemsInCategory(cat);
    CATEGORIES = CATEGORIES.filter(c => c !== cat);
    if(next && !CATEGORIES.includes(next)) CATEGORIES.push(next);
    await persistCategories();
    // บันทึกพร้อมกันทุกเมนู (ทีละเมนูจะช้ามากเพราะแต่ละคำขอใช้เวลาเป็นร้อยมิลลิวินาที) และไม่ต้องเด้ง toast รายเมนู
    await Promise.all(affected.map(it => { ensureEdited(it.id).category = next; return persist(it.id, false); }));
    if(currentCategoryFilter === cat) currentCategoryFilter = next || null;
    renderAll();
    showToast(okMsg + ' (' + affected.length + ' เมนู)');
  }
  async function renameCategory(cat){
    const input = window.prompt('เปลี่ยนชื่อหมวดหมู่ "' + cat + '" เป็น:', cat);
    if(input === null) return;
    const next = input.trim();
    if(!next || next === cat) return;
    if(CATEGORIES.includes(next) && !window.confirm('มีหมวดหมู่ "' + next + '" อยู่แล้ว จะรวมเมนูของ "' + cat + '" เข้าไปด้วยกันหรือไม่?')) return;
    await applyCategoryChange(cat, next, 'เปลี่ยนชื่อเป็น "' + next + '" แล้ว');
  }
  async function deleteCategory(cat){
    const n = itemsInCategory(cat).length;
    if(!window.confirm('ลบหมวดหมู่ "' + cat + '"? ' + (n ? 'เมนู ' + n + ' รายการในหมวดนี้จะกลายเป็น "ไม่ระบุหมวดหมู่" (ไม่ได้ลบเมนู)' : 'หมวดนี้ยังไม่มีเมนูอยู่'))) return;
    await applyCategoryChange(cat, '', 'ลบหมวดหมู่ "' + cat + '" แล้ว');
  }
    ...
    filtered.forEach(it => {
      ...
      row.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:6px;">
          <div class="title">${it.verified ? '<span class="verified-badge" title="ตรวจสอบแล้ว">✓</span>' : ''}${escapeHtml(it.clean_title)}</div>
  ...
  function renderOverview(){
    ...
    items.forEach(it => {
      ...
      rows += `<tr class="clickable" data-id="${it.id}">
        ...
        <td class="verify-cell">${it.verified ? '<span class="verified-badge" title="ตรวจสอบแล้ว">✓</span>' : '—'}</td>
    ...
    main.innerHTML = `
      <div class="overview">
        ...
        <table class="ov-table">
          <thead><tr><th>เมนู</th><th>หมวดหมู่</th><th>ตรวจสอบ</th><th>รหัสสินค้า</th><th class="num">จำนวนคน</th><th class="num">ความเร็วสายพาน (Hz)</th><th class="num">ไม้กั้น</th><th class="num">Rate (pc/hr)</th><th class="num">รูปตำแหน่ง</th><th></th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>`;
  ...
  function renderDetail(id){
    ...
    main.innerHTML = `
      <div class="detail-header">
        ...
        <div class="detail-actions">
          <button class="btn btn-outline" id="undoBtn" title="ย้อนกลับการแก้ไขล่าสุด (Ctrl+Z)" ${canUndo(id) ? '' : 'disabled'}>↶ ย้อนกลับ${canUndo(id) ? ' (' + undoStack[id].length + ')' : ''}</button>
          <button class="btn btn-verify ${it.verified ? 'on' : ''}" id="verifyBtn" title="${it.verified ? 'กดเพื่อยกเลิกสถานะตรวจสอบแล้ว' : 'กดหลังตรวจสอบมาตรฐานของเมนูนี้เรียบร้อยแล้ว'}">${it.verified ? '✓ ตรวจสอบแล้ว' + (verifiedDateText(it) ? ' · ' + verifiedDateText(it) : '') : '☐ ยังไม่ตรวจสอบ'}</button>
      ...
      <div class="belt-zoombar">
        ...
        <button class="btn btn-outline btn-small" id="exportImgBtn" title="บันทึกภาพสายพานทั้งไลน์เป็นไฟล์ PNG">🖼 บันทึกเป็นรูป</button>
      </div>
    ...
    document.getElementById('verifyBtn').onclick = () => toggleVerified(id);
    const undoBtn = document.getElementById('undoBtn');
    // กันช่องที่กำลังพิมพ์ blur ตอนกดปุ่ม — blur จะบันทึก+วาดหน้าใหม่ ปุ่มที่กดอยู่หายไปก่อน click จะทำงาน
    undoBtn.addEventListener('mousedown', (e)=> e.preventDefault());
    undoBtn.onclick = () => undoLast(id);
    // พิมพ์ค้างไว้ก็ย้อนได้ (แค่เปิดปุ่ม ไม่ต้อง stringify ทั้งเมนูทุกครั้งที่กดแป้น)
    // ใช้ property แทน addEventListener — #main อยู่ถาวร ถ้า add ทุกครั้งที่ render listener จะสะสม
    main.oninput = ()=>{ const b = document.getElementById('undoBtn'); if(b) b.disabled = false; };
    ...
    document.getElementById('exportImgBtn').onclick = () => exportBeltImage(id);
    ...
    rememberState(id); // หลังแปลงตำแหน่งแล้ว จะได้ไม่นับการแปลงเป็นการแก้ไขหนึ่งครั้ง
  }
  ...
  // Ctrl+Z / ⌘Z ย้อนกลับ — ยกเว้นตอนอยู่ในช่องพิมพ์ ปล่อยให้เป็นการย้อนตัวอักษรของเบราว์เซอร์ตามปกติ
  document.addEventListener('keydown', (e)=>{
    if(!(e.ctrlKey || e.metaKey) || e.shiftKey || e.altKey || (e.key || '').toLowerCase() !== 'z') return;
    if(e.target.closest && e.target.closest('input, textarea, select, [contenteditable="true"]')) return;
    if(!currentId || !document.getElementById('undoBtn')) return;
    e.preventDefault();
    undoLast(currentId);
  });
  document.addEventListener('keydown', (e)=>{
  ...
  applyLoadedEdits({catRes, delRes, idxRes, itemRows});
