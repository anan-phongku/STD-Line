...
export async function initApp(){
  ...
  // กรอบของเครื่องจักร: ซ้าย/กว้างเป็น % ของความกว้างราง (ราง 3 เครื่องเรียงกันได้ทั้งบนคอมและมือถือ)
  // ส่วนสูงเป็น px เหมือน topPx — ของเก่าที่ยังไม่มีค่าเหล่านี้จะถูกวางกลางรางด้วยขนาดมาตรฐาน
  const MACHINE_W_DEFAULT = 31;
  function machineBox(m){
    const w = Number(m && m.wPct) > 0 ? Number(m.wPct) : MACHINE_W_DEFAULT;
    const left = (m && isFinite(parseFloat(m.leftPct))) ? parseFloat(m.leftPct) : (50 - w / 2);
    const h = (m && Number(m.hPx) > 0) ? Number(m.hPx) : null;
    return {left, w, h};
  }
  ...
  function renderOverlay(belt){
    ...
    machines.forEach((m, idx) => {
      ...
      html += `<div class="ov-machine" data-machine="${idx}" style="top:${ovTop(m)};left:${box.left}%;width:${box.w}%;${box.h ? 'height:' + box.h + 'px;' : ''}">
        <div class="machine-resize" data-machineresize="${idx}" title="ลากเพื่อปรับขนาด"></div>
  ...
  function setupDragging(id){
    ...
    stage.querySelectorAll('.machine-head').forEach(el=>{
      el.addEventListener('mousedown', (e)=> startMachineDrag(e, id, parseInt(el.dataset.machinedrag), overlay, 'move'));
      el.addEventListener('touchstart', (e)=> startMachineDrag(e, id, parseInt(el.dataset.machinedrag), overlay, 'move'), {passive:false});
    });
    stage.querySelectorAll('.machine-resize').forEach(el=>{
      el.addEventListener('mousedown', (e)=> startMachineDrag(e, id, parseInt(el.dataset.machineresize), overlay, 'resize'));
      el.addEventListener('touchstart', (e)=> startMachineDrag(e, id, parseInt(el.dataset.machineresize), overlay, 'resize'), {passive:false});
    });
  }
  
  // เครื่องจักรลากได้ทั้งสองแกน และปรับขนาดที่มุมขวาล่าง
  // ระหว่างลากแก้แค่ style ของกล่องเดิม ไม่วาด DOM ใหม่ (วาดใหม่แล้ว touch บนมือถือหลุด)
  const MACHINE_MIN_W = 100, MACHINE_MIN_H = 96;
  function startMachineDrag(e, id, idx, ref, mode){
    ...
    function onMove(ev){
      ...
      if(mode === 'move'){
        const left = Math.max(0, Math.min(W - start.w, start.left + dx));
        const top = Math.max(0, Math.min(Math.max(0, H - start.h), start.top + dy));
        m.leftPct = Math.round(left / W * 1000) / 10;
        m.topPx = Math.round(top); m.top = top / H * 100;
        if(!(Number(m.wPct) > 0)) m.wPct = Math.round(start.w / W * 1000) / 10;
        item.style.left = m.leftPct + '%'; item.style.top = m.topPx + 'px';
      }else{
        const w = Math.max(MACHINE_MIN_W, Math.min(W - start.left, start.w + dx));
        const h = Math.max(MACHINE_MIN_H, start.h + dy);
        m.wPct = Math.round(w / W * 1000) / 10;
        m.hPx = Math.round(h);
        if(!isFinite(parseFloat(m.leftPct))) m.leftPct = Math.round(start.left / W * 1000) / 10;
        item.style.width = m.wPct + '%'; item.style.height = m.hPx + 'px';
        item.style.left = m.leftPct + '%';
      }
    }
    function onUp(){
  ...
  function addMachine(id){
    const h = beltLayoutHeight();
    const ed = ensureEdited(id);
    const topPx = h ? Math.round(h*0.4) : undefined;
    const w = MACHINE_W_DEFAULT;
    const cols = [50 - w/2, 1.5, 100 - w - 1.5];
    const taken = (ed.belt.machines || []).filter(m => m && (topPx === undefined || Math.abs((m.topPx || 0) - topPx) < 60));
    const leftPct = cols.find(c => !taken.some(m => Math.abs(machineBox(m).left - c) < w/2)) ?? cols[0];
    ed.belt.machines.push({id:uid(), top:40, topPx, leftPct, wPct:w, name:'', people:'', rate:'', type:'person', img:null});
    persist(id); renderDetail(id); renderSidebar(document.getElementById('searchBox').value);
  }
