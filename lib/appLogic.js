...
export async function initApp(){
  ...
  function readStationClip(){
    try{ const raw = window.localStorage.getItem(CLIP_KEY); return raw ? JSON.parse(raw) : null; }catch(e){ return null; }
  }
  ...
  // สร้างตารางกระบวนการจากสถานีบนสายพาน — นับ "ทุกสถานี" ของทั้งสองเลน
  ...
  function renderSidebar(filter){
    ...
    filtered.forEach(it => {
      ...
      row.innerHTML = `
        ...
        <div class="meta">
          <span>👥 ${fmtPeople(sumPeople(computeSteps(it)))} คน</span>
          <span>⚡ ${fmtPeople(it.summary.belt_speed)} Hz</span>
          <span>▮ ${belt.bars.length} ไม้กั้น</span>
          <span>⏱ ${lineRate(it)===''?'—':lineRate(it)} pc/hr</span>
  ...
  function stationRowHtml(side, idx, s, total){
    const btns = `<div class="station-btns">
          ...
          <button class="station-copy-btn" data-copystation="${side}:${idx}" title="คัดลอกสถานีนี้ไปวางที่เมนูอื่นได้">⧉ คัดลอก</button>
        </div>`;
    ...
    return `
      <div class="head-card ${s.blank?'blank':''}" draggable="true" data-dragside="head" data-dragidx="${idx}">
        <div class="head-card-top">
          ...
          <button class="station-copy-btn" data-copystation="head:${idx}" title="คัดลอกสถานีนี้ไปวางที่เมนูอื่นได้">⧉</button>
  ...
  function pasteBtnHtml(side, full){
    const clip = readStationClip();
    if(!clip) return '';
    const disabled = full ? 'disabled' : '';
    return `<button class="btn btn-outline btn-small lane-paste-btn" data-pasteslot="${side}" ${disabled} title="วางสถานีที่คัดลอกไว้ลงตรงนี้">📋 วาง "${escapeHtml(clipName(clip))}"</button>`;
  }
  function renderHeadRow(headSlots){
    const cards = headSlots.map((s,i)=> headCardHtml(i, s, headSlots.length)).join('');
    const full = headSlots.length>=15;
  ...
  function renderLane(side, slots){
    ...
    return `
      ...
      ${pasteBtnHtml(side, slots.length>=15)}`;
  }
  ...
  function renderOverlay(belt){
    ...
    machines.forEach((m, idx) => {
      if(!m) return;
      html += `<div class="ov-machine" data-machine="${idx}" style="top:${m.top}%;">
  ...
  function renderDetail(id){
    ...
    main.querySelectorAll('[data-copystation]').forEach(b=> b.onclick = (e)=>{
      ...
      copyStation(id, side, parseInt(sidx));
    });
  ...
  function startDrag(e, id, kind, idx, ref){
    ...
    function onMove(ev){
      ...
      if(kind==='bar'){
        ...
        m.top = Math.min(88, top); // กล่องสูง 3 ช่อง กันไม่ให้ดันเลยท้ายราง
      } else {
  ...
  function renderOverlayOnly(id){
    ...
    if(ov) ov.innerHTML = renderOverlay(belt);
  }
  ...
  function copyStation(id, side, idx){
    ...
    delete clip.id;
    try{ window.localStorage.setItem(CLIP_KEY, JSON.stringify(clip)); }
    catch(e){ showToast('คัดลอกไม่สำเร็จ (พื้นที่เก็บในเครื่องเต็ม)'); return; }
    showToast('คัดลอกสถานี "' + clipName(clip) + '" แล้ว — เปิดเมนูที่ต้องการแล้วกดปุ่ม "วาง"');
    renderDetail(id); // ให้ปุ่มวางโผล่ขึ้นมาทันที
  }
  function pasteStation(id, side){
    ...
    arr.push({...JSON.parse(JSON.stringify(clip)), id: uid()});
    syncStationsToSteps(id);
    persist(id, 'วางสถานี "' + clipName(clip) + '" แล้ว');
  ...
  function addMachine(id){
    const ed = ensureEdited(id); ed.belt.machines.push({id:uid(), top:40, name:'', people:'', rate:'', type:'person', img:null});
    persist(id); renderDetail(id); renderSidebar(document.getElementById('searchBox').value);
  }
